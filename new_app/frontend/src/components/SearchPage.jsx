...
function SearchPage() {
  ...
  const handleAddBook = async (book) => {
    try {
      ...
      // Optionally, navigate to the book's new detail page
        ...
        {results.map((book) => (
          <div key={book.isbn} className="flex items-center justify-between p-4 bg-slate-800 rounded-lg">
            <div className="flex items-center gap-4">
              <img
                src={book.coverid ? `https://covers.openlibrary.org/b/id/${book.coverid}-S.jpg` : 'https://via.placeholder.com/50x75.png?text=N/A'}
                alt={book.title}
            ...
            <button
              ...
              className="px-4 py-1 font-semibold text-sm bg-blue-600 text-white rounded-md hover:bg-blue-700"
            >
              Add to Library
            </button>
          </div>
